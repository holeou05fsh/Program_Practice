...
num_qu = random.sample(range(len(a[single_list[num_sw[0]]])),2)
num_qu1 = random.sample(range(len(a[single_list[num_sw[1]]])),2)
num_qu2 = random.sample(range(len(a[single_list[num_sw[2]]])),2)
num_qu3 = random.sample(range(len(a[single_list[num_sw[3]]])),2)
num_qu4 = random.sample(range(len(a[single_list[num_sw[4]]])),2)
...
l2 = tk.Label(window,bg='skyblue',font=('新細明體',28)
             ,width=90,height=5)
l2.place(x=0,y=430)
l3 = tk.Label(window,bg='skyblue',fg='red',font=('新細明體',36),text='每日五單字測試').place(x=36,y=36)
