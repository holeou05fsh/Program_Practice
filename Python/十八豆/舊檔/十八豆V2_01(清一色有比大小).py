...
if b:
    ...
    W_PF = []
    W_PF1 = []
    W_P = [v for i, v in enumerate(player_statistics) if v[0] == '一']
    for i in W_P:
        W_PF.append(W_P[b][-1])
        b += 1
    # print(W_PF)
    W_PF = list(map(int, W_PF))

    W_P1 = [i for i, v in enumerate(player_statistics) if v[-1] == str(max(W_PF))]
    for i in W_P1:
        print('※恭喜 玩家' + str(int(W_P1[a]) + 1), '獲勝')
        a += 1
